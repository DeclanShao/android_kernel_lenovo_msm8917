...
/* add by wangshuai4 for modify shutdown charger current */
#ifdef CONFIG_MACH_LENOVO_TB8804
static bool gf_is_bootdevice(struct gf_dev *gf_dev)
{
...
/* add by wangshuai4 for modify shutdown charger current */
#ifdef CONFIG_MACH_LENOVO_TB8804
	if(gf_is_bootdevice(gf_dev)){
	printk("gf is err,boot is charging,wangshuai\n");
		goto error;	
	}else{
        printk("gf is ok,boot is open,wangshuai\n");
    }
#endif
