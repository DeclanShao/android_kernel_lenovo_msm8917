...
#define SX9310_VDD_MIN_UV       3300000
#define SX9310_VDD_MAX_UV       3300000
#define SX9310_VDD1_MIN_UV       3300000
#define SX9310_VDD1_MAX_UV       3300000
#define SX9310_VIO_MIN_UV       1800000
#define SX9310_VIO_MAX_UV       1800000

#define SX9310_SENSOR_ID      0X42
...
static psx9310_t PSX9310Device = 0;

static unsigned int sx9310_enable = 1;

static unsigned int sx9310_status = 0;

#define PROC_CAPSENSOR_FILE "capsensor_enable"
psx93XX_t psx93XX_this = 0;
static ssize_t capsensor_config_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t capsensor_config_write_proc(struct file *, const char __user *, size_t, loff_t *);
static struct proc_dir_entry *capsensor_proc = NULL;
...
static ssize_t sx9310_status_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	psx93XX_t this = dev_get_drvdata(dev);

	dev_dbg(this->pdev, "Reading sx9310  status.\n");
	return sprintf(buf, "0x%x\n", sx9310_status);
}

static DEVICE_ATTR(status, 0664, sx9310_status_show,NULL);
static ssize_t sx9310_sensor_ID_show(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	psx93XX_t this = dev_get_drvdata(dev);
            u8 sensorid = 0;
         read_register(this,SX9310_SENSOR_ID,&sensorid);

	
	return sprintf(buf, "%d\n",sensorid);
}

static DEVICE_ATTR(capsensorid, 0664, sx9310_sensor_ID_show,NULL);
	...
	if (sscanf(buf, "%u", &val) != 1)
  	...
  	if (val) {
		if (!sx9310_enable && this) {
			...
			if(this)
				dev_info( this->pdev, "sx9310 has already enabled.\n");
		}
  	} else {
		if(sx9310_enable && this) {
			...
			if(this)
				dev_info( this->pdev, "sx9310 has already disabled.\n");
    	}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ssize_t len = 0;
        ...
        while ( i < reg_num){        
		err = read_register(this, sx9310_i2c_reg_setup[i].reg,&reg_value);
  	...
  	if (this && (pDevice = this->pDevice)) {
    	...
    	for (counter = 0; counter < numberOfButtons; counter++) {
      		...
      		switch (pCurrentButton->state) {
				...
				//pr_err("yxw %s:IDLE !!!counter=%d,i=0x%x,mask=0x%x\n",__func__,counter,i,pCurrentButton->mask);
            			input_report_key(input, pCurrentButton->keycode, 1);
				sx9310_status = pCurrentButton->keycode;
				//pr_err("yxw %s:IDLE !!!sx9310_status=0x%x\n",__func__,sx9310_status);
            			pCurrentButton->state = ACTIVE;
          			} else {
...
{
	int ret;
	data->vdd = regulator_get(&((struct i2c_client *)data->bus)->dev, "vdd");
	if (IS_ERR(data->vdd)) {
	...
	data->vdd1 = regulator_get(&((struct i2c_client *)data->bus)->dev, "vdd1");
	if (IS_ERR(data->vdd1)) {
	...
	data->vio = regulator_get(&((struct i2c_client *)data->bus)->dev, "vio");
	if (IS_ERR(data->vio)) {
...
static ssize_t capsensor_config_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *ptr = page;
...
static ssize_t capsensor_config_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
    unsigned int val = 0;
	...
	pr_info("%s val = %u\n", __func__, val);
  	if (val) {
		if (!sx9310_enable && psx93XX_this) {
			...
			write_register(psx93XX_this,0x10,0x56);
		} else {
			if(psx93XX_this)
...
static int sx9310_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!pplatData) {
  	...
  	if (gpio_is_valid(pplatData->irq_gpio)) {
		...
		if (err < 0) {
			dev_err(&client->dev, "gpio req failed for sx9310 interrupt\n");
			pplatData->irq_gpio = INVALID_GPIO;
			goto error_0;
		}
		err = gpio_direction_input(pplatData->irq_gpio);
		if (err) {
			dev_err(&client->dev,"unable to set direction for gpio " \
					"[%d]\n", pplatData->irq_gpio);
			goto error_0;
		}
  	}
  	...
  	if (this) {
		...
		capsensor_proc = proc_create(PROC_CAPSENSOR_FILE, 0660, NULL, &config_proc_ops);
		...
		{
			pr_info("create proc entry %s success", PROC_CAPSENSOR_FILE);
		}
